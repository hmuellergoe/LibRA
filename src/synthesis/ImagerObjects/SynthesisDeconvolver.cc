  ...
  void SynthesisDeconvolver::setupDeconvolution(const SynthesisParamsDeconv& decpars)
  {
	...
	if(decpars.algorithm==String("hogbom"))
      ...
      if (decpars.nTaylorTerms > 1) // wide-band
	    ...
	    itsDeconvolver.reset(new SDAlgorithmAAspClean(decpars.scales, decpars.hogbomGain, decpars.fusedThreshold, isSingle, decpars.largestscale, -1, decpars.lbfgsEpsF, decpars.lbfgsEpsX, decpars.lbfgsEpsG, decpars.lbfgsMaxit));
	  }
	else if (decpars.algorithm==String("waveletasp"))
	  {
      bool isSingle = false;
      if (decpars.specmode == String("mfs"))
        isSingle = true;
	  ...
	  itsDeconvolver.reset(new SDAlgorithmSpectralAspClean(decpars.scales, decpars.hogbomGain, decpars.fusedThreshold, isSingle, decpars.largestscale, -1, decpars.lbfgsEpsF, decpars.lbfgsEpsX, decpars.lbfgsEpsG, decpars.lbfgsMaxit));
	  }
  /*else if (decpars.algorithm==String("asp")) 
    {
      bool isSingle = false;
